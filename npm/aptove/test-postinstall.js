#!/usr/bin/env node
/**
 * Tests for aptove npm postinstall script.
 * Uses only Node.js built-ins — no external dependencies required.
 *
 * Run: node npm/aptove/test-postinstall.js
 */
...
const SHORT_NAME = (PLATFORM_PACKAGES[PLATFORM_KEY] || '@aptove/aptove-unknown').split('/')[1];

/**
 * Creates a temp directory simulating the node_modules layout:
 *   <tmp>/node_modules/@aptove/aptove/          ← baseDir
 *   <tmp>/node_modules/@aptove/<platform>/bin/  ← sibling binary
 *
 * binaryVersion: if non-null, creates a fake binary that prints "aptove <version>"
 * pkgVersion:    version written into optionalDependencies
 */
function makeLayout({ binaryVersion = null, pkgVersion = '1.2.3' } = {}) {
  const tmp = fs.mkdtempSync(path.join(os.tmpdir(), 'aptove-postinstall-test-'));
  const baseDir = path.join(tmp, 'node_modules', '@aptove', 'aptove');
...
function writeFakeBinary(binaryPath, version) {
  if (process.platform === 'win32') {
    fs.writeFileSync(binaryPath, `@echo aptove ${version}\r\n`);
  } else {
    fs.writeFileSync(binaryPath, `#!/bin/sh\nprintf 'aptove ${version}'\n`, { mode: 0o755 });
  }
}
